...
export function CharacterReferences({
  ...
  async function handleUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      for (const file of Array.from(files)) {
        const path = `${userId}/${projectId}/characters/${characterId}/${crypto.randomUUID()}-${file.name}`;
        const { error: uploadError } = await supabase.storage.from("assets").upload(path, file);
        if (uploadError) throw uploadError;

        const { data: inserted, error: insertError } = await supabase
          .from("character_references")
          .insert({ character_id: characterId, storage_path: path, is_primary: references.length === 0 })
  ...
  async function handleDelete(ref: ReferenceRow) {
    ...
    await supabase.from("character_references").delete().eq("id", ref.id);
    ...
    <div className="flex flex-col gap-3">
      ...
      {references.length === 0 ? (
        <p className="text-xs text-muted-foreground">
          No reference images yet — add some so generated scenes stay consistent.
        </p>
      ) : (
